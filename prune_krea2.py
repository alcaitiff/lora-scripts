...
KREA2/Flux flavor — understands diffusion_model.blocks.<N>, txtfusion.layerwise_blocks.<N>,
and txtfusion.refiner_blocks.<N> key formats.
...
Usage:
  ./prune_krea2.py --match ".attn." ".blocks.20." file1.safetensors
  ./prune_krea2.py --match ".attn." "wk" name*.safetensors
  ./prune_krea2.py --match ".attn." --dry-run "name*.safetensors"
  ./prune_krea2.py --match ".attn." --blocks 4 7 10-13 lora.safetensors
  ./prune_krea2.py --blocks 4 7 10-13 lora.safetensors
  ./prune_krea2.py --match ".attn." input.safetensors --out pruned.safetensors
  ./prune_krea2.py --match "!wk" "!wq" input.safetensors   # keep-only mode
"""
...
parser = argparse.ArgumentParser(
    description="Prune KREA2/Flux LoRA tensors by key substring match (supports keep-only mode via '!')"
)
parser.add_argument(
    "--match",
    nargs="+",
...
parser.add_argument(
    "--blocks",
    nargs="+",
    help="Block indexes or ranges (e.g. 4 7 10-13) to match as .blocks.<N>., .layerwise_blocks.<N>., .refiner_blocks.<N>.",
)
parser.add_argument(
    "inputs",
    nargs="*",
...
if args.blocks:
    ...
    block_matches = []
    for i in uniq:
        # KREA2 / Flux key patterns:
        #   diffusion_model.blocks.<i>.
        #   diffusion_model.txtfusion.layerwise_blocks.<i>.
        #   diffusion_model.txtfusion.refiner_blocks.<i>.
        block_matches.append(f".blocks.{i}.")
        block_matches.append(f".layerwise_blocks.{i}.")
        block_matches.append(f".refiner_blocks.{i}.")
